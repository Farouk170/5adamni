...
class postsC{
    ...
    public function InsererPosts($content, $file_path,$id_user) {
        $pdo = openDB();
    
        try {
            // Prepare the SQL statement
            if(!empty($file_path)){
                $statement = $pdo->prepare("INSERT INTO posts (Content, MediaData, user_id) VALUES (:content, :media_data, :id_user)");
                $statement->bindParam(':media_data', $file_path);
            }
            else{
                $statement = $pdo->prepare("INSERT INTO posts (Content, user_id) VALUES (:content, :id_user)");
            }
            // Bind parameters
    ...
    public function updatepostContent($postId, $editedContent) {
        try {
            $pdo = openDB();
            
            $statement = $pdo->prepare("UPDATE posts SET content = :editedContent WHERE PostID = :postId");
