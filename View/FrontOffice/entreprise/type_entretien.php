...
<div class="site-wrap">
        ...
        <div class="row align-items-center">
          ...
          <nav class="mx-auto site-navigation col-6">
            <ul class="site-menu js-clone-nav d-none d-xl-block ml-0 pl-0">
              ...
              <li><a></a></li>
              <li class="has-children">
                <a href="entretiens_c.php" class="nav-link active">Types entretiens</a>
                <ul class="dropdown">
          ...
          <div class="col-md-12">
              ...
              <div class="container-fluid pt-4 px-4 table-container">                    
                <div class="bg-white text-center rounded p-4">
                  <div class="d-flex align-items-center justify-content-between mb-4">
                    ...
                    <h4 class="mb-0"><u style="text-decoration:none;">Les différents types d'entretien</u></h4><br><br>
                    <a href="add_type.php" style="text-decoration:none;" class="btn">Ajouter <i class="bi-person-plus-fill"></i></a>                          
                </div>
            ...
            <table class="table text-start align-middle table-bordered table-hover mb-0">
              ...
              <?php
                ...
                // Détermination de l'index de départ pour la pagination
                $itemsPerPage = 5;
                $startIndex = ($page - 1) * $itemsPerPage;

                try {
                    // Instanciation d'un objet de la classe TypeController
                    $typeController = new TypeController();

                    // Récupération du nombre total de types d'entretiens
                    $totalTypes = $typeController->countTypes();

                    // Calcul du nombre total de pages
                    $totalPages = ceil($totalTypes / $itemsPerPage);

                    // Récupération des types d'entretiens pour la page actuelle
                    $types = $typeController->readTypes($startIndex, $itemsPerPage);

                    // Affichage des données dans le tableau
                    if (!empty($types)) {
                        foreach ($types as $type) {
                            echo "<tr>";
                            echo "<td>" . htmlspecialchars($type['type_entretien']) . "</td>";
                            echo "<td>" . htmlspecialchars($type['description']) . "</td>";
                            echo "<td><button class='btn btn-outline-modify'><a href='modifier_type.php?id=" . $type['id_te'] . "' style='text-decoration: none; color: white;'>Modifier</a></button></td>";
                            echo "<td><button class='btn btn-sm btn-primary'><a href='#' onclick='confirmerSuppression(" . $type['id_te'] . ")' style='text-decoration: none;color: white;'>Supprimer</a></button></td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='4'>Aucun type d'entretien trouvé.</td></tr>";
                    }
    ...
    <script>
    function confirmerSuppression(id_te) {
        if (confirm("Êtes-vous sûr de vouloir supprimer ce type ?")) {
            var xhr = new XMLHttpRequest();
            xhr.open("GET", "supprimer_type.php?id_te=" + id_te, true);
